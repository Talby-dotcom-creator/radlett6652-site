...
const MediaManager: React.FC<MediaManagerProps> = ({
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      if (!bucketExists) {
        throw new Error(
          `Storage bucket '${STORAGE_BUCKET}' does not exist. Please create it first.`
        );
      }
      ...
      const uploadPromises = Array.from(files).map(async (file) => {
        ...
        const uploadResult = (await Promise.race([
          supabase.storage.from(STORAGE_BUCKET).upload(uniqueFilename, file, {
            cacheControl: "3600",
            upsert: false,
          }),
  ...
  const extractCategory = (item: MediaItem): string => {
    ...
    if (filename.includes("gpc") && filename.includes("minutes"))
      return "gpc_minutes";
    if (filename.includes("minutes")) return "minutes";
    if (filename.includes("grand") || filename.includes("ugle"))
      return "grand_lodge";
    if (filename.includes("provincial") || filename.includes("pgl"))
      return "provincial";
    return "other";
  };
  ...
  const getCategoryCounts = () => {
    const documentItems = mediaItems.filter((item) => {
      ...
      if (
        selectedYear !== "all" &&
        extractYear(item) !== parseInt(selectedYear)
      )
        return false;
      return true;
    });
    ...
    Object.keys(DOCUMENT_CATEGORIES).forEach((category) => {
      counts[category] = documentItems.filter(
        (item) => extractCategory(item) === category
      ).length;
    });
    return counts;
  };
  ...
  // Filter items
  const filteredImages = mediaItems.filter(
    (item) => item.type === "image" && (filter === "all" || filter === "images")
  );

  const filteredDocuments = mediaItems
    .filter((item) => {
      if (item.type !== "document") return false;
      ...
      if (
        ...
        extractCategory(item) !== selectedCategory
      )
        return false;
      return true;
    })
              ...
              >
                <Upload
                  className={`w-12 h-12 mx-auto mb-4 ${
                    uploading ? "text-neutral-300" : "text-neutral-400"
                  }`}
                />
                <p
                  className={`mb-2 ${
                    uploading ? "text-neutral-400" : "text-neutral-600"
                  }`}
                >
                  {uploading
                    ? "Uploading files..."
                  ...
                  <div>
                    ...
                    >
                      ...
                      {availableYears.map((year) => {
                        const yearCount = mediaItems.filter(
                          (item) =>
                            item.type === "document" &&
                ...
                <div className="flex items-center space-x-2 text-sm">
                  ...
                  {selectedCategory !== "all" && (
                    <span className="bg-secondary-100 text-secondary-700 px-2 py-1 rounded text-xs">
                      {
                        DOCUMENT_CATEGORIES[
                          selectedCategory as keyof typeof DOCUMENT_CATEGORIES
                        ]?.label
                      }
                    </span>
                  )}
