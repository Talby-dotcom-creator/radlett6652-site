/*
  ...
  This migration ensures all required tables exist with proper structure and policies.
  It's safe to run multiple times as it uses IF NOT EXISTS clauses.

  1. Tables Created:
    - member_profiles (with role column and constraints)
    - lodge_documents 
    - meeting_minutes

  2. Security:
    - Enable RLS on all tables
    - Comprehensive policies for members and admins
...
DECLARE
  admin_user_id UUID;
BEGIN
  -- Check if there's a user with the admin email
  SELECT id INTO admin_user_id 
  FROM auth.users 
  WHERE email = 'ptalbot37@fastmail.com' 
  LIMIT 1;
  
  -- If admin user exists but no profile, create one
