// src/pages/CMSAdminPage.tsx (restored preview)
...
const CMSAdminPage: React.FC = () => {
   ...
   *  EVENTS CRUD
   * ----------------------------------------------------- */
  const handleCreateEvent = async (event: Partial<CMSEvent>) => {
  ...
  const handleUpdateEvent = async (id: string, event: Partial<CMSEvent>) => {
  ...
  const handleUpdatePageContent = async (newContent: string) => {
    ...
    await cmsApi.updatePageContent(pageContent.id, { content: newContent });
