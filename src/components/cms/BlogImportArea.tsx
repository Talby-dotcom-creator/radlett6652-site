...
const BlogImportArea: React.FC<BlogImportAreaProps> = ({
  onContentImported,
  className = "",
}) => {
  ...
  const parseMarkdownContent = async (
    ...
    try {
      // Convert Markdown to HTML (marked may be sync or async depending on version)
  ...
  const parseHTMLContent = async (
    ...
    try {
      ...
      if (metaDescription?.getAttribute("content")) {
        ...
        summary =
          firstP.textContent.trim().substring(0, 200) +
          (firstP.textContent.length > 200 ? "..." : "");
      }
      ...
      const scripts = contentDoc.querySelectorAll(
      ...
      if (!title && !content) {
        return {
          success: false,
          error:
            "Could not extract meaningful content from the HTML file. Please ensure it contains proper HTML structure.",
        };
      }
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const result = isMarkdown
        ? await parseMarkdownContent(content)
        : await parseHTMLContent(content);
      // Ensure content is a string (some DOM operations may return non-string types)
      if (result && result.content) result.content = String(result.content);
    ...
    <div className={`space-y-4 ${className}`}>
          ...
          {isProcessing ? (
            <>
              ...
              <p className="text-sm text-neutral-500 mt-1">
                Extracting content and formatting
              </p>
            </>
          ) : (
            <>
              <Upload
                className={`w-12 h-12 mb-4 ${
                  isDragOver ? "text-secondary-500" : "text-neutral-400"
                }`}
              />
              <p
                className={`mb-2 font-medium ${
                  isDragOver ? "text-secondary-600" : "text-neutral-600"
                }`}
              >
                {isDragOver
                  ? "Drop your file here"
              ...
              <div className="flex-grow">
                ...
                {lastImport.success ? (
                  <div className="text-sm text-green-700 space-y-1">
                    <p>
                      <strong>Title:</strong> {lastImport.title}
                    </p>
                    <p>
                      <strong>Summary:</strong>{" "}
                      {lastImport.summary?.substring(0, 100)}...
                    </p>
                    {lastImport.tags && lastImport.tags.length > 0 && (
                      <p>
                        <strong>Tags:</strong> {lastImport.tags.join(", ")}
                      </p>
                    )}
                    <p className="text-xs mt-2">
                      Content has been populated in the form below. You can
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <div className="text-sm text-blue-700 space-y-1">
          <p>
            <strong>Markdown files (.md):</strong>
          </p>
