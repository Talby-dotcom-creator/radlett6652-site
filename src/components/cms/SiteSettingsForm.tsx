...
const SiteSettingsForm: React.FC<SiteSettingsFormProps> = ({ initialData, onSubmit, onCancel }) => {
  ...
  const groupedSettings = initialData.reduce((acc, setting) => {
    const category = setting.setting_key.includes("contact")
      ? "Contact Information"
      : setting.setting_key.includes("meeting")
      ? "Meeting Information"
      : setting.setting_key.includes("_url")
      ? "Social Media"
      : "General";
  ...
  const renderSettingInput = (setting: SiteSetting) => {
    ...
    const isTextarea =
      setting.setting_key.includes("schedule") || setting.setting_key.includes("location");
